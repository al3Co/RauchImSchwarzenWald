 ...
 * <p>
 * Specifically, this class enables one to compare two double numbers to see if
 * they are within an epsilon of each other.
 *
 */
public class FloatingPoint {
	 ...
	 *
	 * @param d1
	 *            first number against which to compare
	 * @param d2
	 *            second number being compared
	 */
	public static int compare(double d1, double d2) {
		if (FloatingPoint.lesser(d1, d2)) {
			return -1;
		}
		if (FloatingPoint.same(d1, d2)) {
			return 0;
		}
	...
	/**
	 * When value won't work, because numbers are potentially infinite, then use
	 * this one.
	 * <p>
	 * Standarard means for comparing double when dealing with the special
	 * quantities, NaN and infinite numbers. Also properly ensures that numbers
	 * "close to zero" (within an epsilon) are to be treated as zero for this
	 * computation.
	 *
	 * @param d1
	...
	public static boolean same(double d1, double d2) {
		// NaN numbers cannot be compared with '==' and must be treated
		// separately.
		if (Double.isNaN(d1)) {
	...
	/**
	 * See if the value is close enough to actually be considered 0.0 and return
	 * 0.0 if need be.
	 * <p>
